...
const renderAvatar = (name, size) => {
  ...
  if (name === undefined || name === '') {
    ...
    content = <Icon name={AVATAR_ICON} size={avatarIconSize} color={Colors.white} />;
  } else {
    const a = name.split(' ');
    ...
    content = (
      <Text
        style={[
          styles.initials,
          (size === 'small' || (size > 30 && size <= 50)) && styles.smallInitials,
...
// Avatar
const Avatar = ({
  borderRadius, color, imageUri, name, rounded, size
}) => {
